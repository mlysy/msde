...
// drift function
inline void hestModel::sdeDr(double *dr, double *x, double *theta) {
...
// diffusion function
inline void hestModel::sdeDf(double *df, double *x, double *theta) {
  df[0] = .5 * x[1];
  df[2] = theta[3];
...
// data validator
inline bool hestModel::isValidData(double *x, double *theta) {
  return(x[1] > 0.0);
}

// parameter validator
inline bool hestModel::isValidParams(double *theta) {
  bool isValid;
