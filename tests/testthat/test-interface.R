...
test_that("diff.R == diff.cpp", {
  mxd <- sapply(1:nrow(cases), function(ii) {
    ...
    df.R <- hest.df(x = init$X.R, theta = init$Theta.R)
...
test_that("ll.R == ll.cpp", {
  mxd <- sapply(1:nrow(cases), function(ii) {
    sx <- cases$single.x[ii]
    st <- cases$single.theta[ii]
    init <- input.init(nreps = c(nobs, nreps), sx = sx, st = st)
    ll <- sde.loglik(model = hmod, x = init$X, theta = init$Theta, dt = dT)
    ll.R <- rep(NA, nreps)
    for(jj in 1:nreps) {
      ll.R[jj] <- hest.ll(x = init$X.R[,,jj], theta = init$Theta.R[jj,], dt = dT)
    }
    if(sx && st) {
      ll.R <- ll.R[1]
    }
...
test_that("lpi.R == lpi.cpp", {
  mxd <- sapply(1:nrow(cases), function(ii) {
    sx <- cases$single.x[ii]
    st <- cases$single.theta[ii]
    init <- input.init(nreps = nreps, sx = sx, st = st)
