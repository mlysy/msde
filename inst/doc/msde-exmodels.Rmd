...
This vignette contains a complete description of the sample models found in `msde::example.models()`.
...
\end{split}
$$
where $V_t$ is a latent stochastic volatility process, and $B_{1t}$ and $B_{2t}$ are Brownian motions with $\cor(B_{1t}, B_{2t}) = \rho$.  To improve the accuracy of the numerical discretization scheme used for inference, `msde::example.models` considers the transformed variables $X_t = \log(S_t)$ and $Z_t = 2 V_t^{1/2}$, for which Heston's SDE becomes
$$
\begin{split}
...
$$
The data and parameters are all restricted to be positive.  This model is contained in `msde::example.models(model = "lotvol")`.
...
\end{split}
$$
where $A_t = \gamma_1D_tQ_t + \gamma_2(10-D_t)$ and $\tth = (\rv \theta 8)$, $\theta_i = \log(\gamma_i)$, are various reaction rates.  The data and parameter restrictions for this model are $\tth \in \mathbb R^8$, $\Y_t > 1$, and $D_t < 10$.  This model is contained in `msde::example.models(model = "pgnet")`.
