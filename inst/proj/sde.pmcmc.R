...
sde.pmcmc <- function(model, init, hyper,
  ...
  nmissN <- ndims - init$nvar.obs.m[nrow(init$data)]
  imissN <- ndims-nmissN+1:nmissN
  # burn-in
  if(missing(burn)) burn <- max(.1, 1e3)
  if(burn < 1) burn <- nsamples*burn
  burn <- floor(burn)
  # random walk standard deviations
  rw.sd <- .set.jump(rw.sd, FALSE, param.names, data.names)
  # output
  ...
  for(ii in (-burn+1):nsamples) {
    ...
    full.prop <- .rw.prop(theta.curr, x.curr, rw.sd,
                          fixed.params, nmiss0)
    x.prop <- full.prop[1:ndims]
    theta.prop <- full.prop[ndims+1:nparams] # or return (x,theta), whichever is more conveninent
    # calculate lacc.prop
