...
A $d$-dimensional SDE $\Y_t = (\rv [1t] Y {dt})$ is written as
$$
...
<!-- $$ -->
and $\B_t = (\rv [1t] B {dt})$ is $d$-dimensional Brownian motion.
...
This is accomplished with the function `sde.sim`.  Here we'll generate $N = 50$ observations of the process with initial values $\Y_0 = (71, 79)$, and parameter values $\tth = (.5, .0025,.3)$.
...
# plot data
X <- rbind(c(x0), lvsim$data) # include first observation
tseq <- (1:N-1)*dT # observation times
clrs <- c("black", "red")
par(mar = c(4, 4, 1, 0)+.1)
plot(x = 0, type = "n", xlim = range(tseq), ylim = range(X),
     xlab = "Time (years)", ylab = "Population")
lines(tseq, X[,"H"], type = "o", pch = 16, col = clrs[1])
lines(tseq, X[,"L"], type = "o", pch = 16, col = clrs[2])
