...
#'@export
sde.make.model <- function(ModelFile, PriorFile = NULL,
                           data.names, param.names, ..., debug = FALSE) {
  if(debug) browser()
  #.msdeCppPath <- "C:/Users/Jerome/Documents/R/library/msdeHeaders/cppTemplates"
  sde.model <- list()
  # ok compile c++ code first
  ...
  sde.model <- c(sde.model, list(ndims = ndims, nparams = nparams,
