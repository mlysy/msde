...
sde.pf <- function(model, init, npart, Z) {
  ...
  # dt <- init$dt.m par.index <- init$nvar.obs.m init.data <- init$data
  ...
  ans <- .pf_eval(sdeptr = model$ptr, initParams = init$params, initData = t(init$data),
                dT = init$dt.m, nDimsPerObs = init$nvar.obs.m, NormalDraws = t(Z))
  ans$X <- t(ans$X)
  ans$lwgt <- t(ans$lwgt)
  return(ans)
}
