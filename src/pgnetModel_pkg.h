...
// drift function
inline void pgnetModel::sdeDr(double *dr, double *x, double *theta) {
  double K = 10.0;
...
// diffusion function
inline void pgnetModel::sdeDf(double *df, double *x, double *theta) {
  double K = 10.0;
...
// data validator
inline bool pgnetModel::isValidData(double *x, double *theta) {
  bool isValid;
  double K = 10.0;
  double eps = 0.05;
  isValid = x[0] > 1.0+eps;
...
// parameter validator
inline bool pgnetModel::isValidParams(double *theta) {
  return(true);
}
